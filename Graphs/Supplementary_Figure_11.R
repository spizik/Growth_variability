...
# Evaluates whether species mixture  types differ in intra-site variability by applying Kruskall Wallist test test to difference between mixed and monospecific stands (sites with not avaiable data are not considerred).
# Prints adjusted significance values highlighting significant pairwise contrasts among different species mixture.
evaluate_mixture <- function(input){
  
  ## Testing arguments
  # input = working_aggregated_mean
  # input = subset(working_aggregated_mean, species == "PISY")
  
  input <- subset(input, species_composition %in% c("Mixed", "Monoculture"))
  input <- input[!is.na(input$cv_RWI) & !is.na(input$species_composition), ]
  
  test <- kruskal.test(cv_RWI ~ species_composition, data = input)
  
  results <- data.frame(
    Statistic = unname(test$statistic),
    P.unadj = test$p.value,
    P.adj = ifelse(test$p.value < 0.05, "significant", "non-significant"),
...
plot_mixture <- function(input){
  # input=working_aggregated_mean
  
  sites_per_zone <- input %>% 
    distinct(site_code, species_composition) %>% 
    dplyr::count(species_composition) %>% 
    dplyr::rename(n_sites = n)
  sites_per_zone$y <- 0.95
  
  input$x <- NA ; sites_per_zone$x <- NA
  input$x[which(input$species_composition == "Mixed")] <- 1 ; sites_per_zone$x[which(sites_per_zone$species_composition == "Mixed")] <- 1
  input$x[which(input$species_composition == "Monoculture")] <- 2 ; sites_per_zone$x[which(sites_per_zone$species_composition == "Monoculture")] <- 2
  input$x[which(input$species_composition == "No_data")] <- 3 ; sites_per_zone$x[which(sites_per_zone$species_composition == "No_data")] <- 3
  input$x[is.na(input$species_composition)] <- 4 ; sites_per_zone$x[is.na(sites_per_zone$species_composition)] <- 4
  
  g <- ggplot(input)
  g <- g + geom_boxplot(aes(x = x, y = cv_RWI, fill = species_composition), alpha = 0.75)
  g <- g + geom_label(data = sites_per_zone, aes(x = x, y = y, label = n_sites, colour=species_composition), fontface = "bold", size = 4, fill="#FFFFFF", alpha=0.40, label.size = 0 )
  g <- g + xlab("") + ylab("Within-site variability")
  g <- g + scale_fill_manual(values = mix_cols, breaks = names(mix_cols)) 
  g <- g + scale_colour_manual(values = mix_cols, breaks = names(mix_cols)) 
  g <- g + scale_x_continuous(limits = c(0.5, 2.5), breaks = c(1:4), labels = c("Mixed", "Monoculture", "No_data", "NA"))
  g <- g + scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, 0.25), labels = formatC(seq(0, 1, 0.25), format = "f", digits = 2))
  g <- my.theme(g)
  g
}
...
working <- merge(working_df[,c("site_code","cv_RWI","species")],
                 site.list[, c("site_code", "data_collection_design", "management_type", "species_composition")],
                 by = "site_code",
                 all.x = TRUE)

working_aggregated_mean <- aggregate(cv_RWI ~ site_code + species + data_collection_design + management_type + species_composition, data = working, FUN = mean)
working_aggregated_sd <- aggregate(cv_RWI ~ site_code + species + data_collection_design + management_type + species_composition, data = working, FUN = sd)
...
print("------------------------------------------------------------------------")
print("------------------------------------------------------------------------")
print("-------------------------- AOV a mean by mixture -----------------------")
print("------------------------------------------------------------------------")
evaluate_mixture(working_aggregated_mean)
print("--------------------- ABAL------------------")
evaluate_mixture(subset(working_aggregated_mean, species == "ABAL"))
print("--------------------- PCAB------------------")
evaluate_mixture(subset(working_aggregated_mean, species == "PCAB"))
print("--------------------- PISY------------------")
evaluate_mixture(subset(working_aggregated_mean, species == "PISY"))
print("--------------------- FASY------------------")
evaluate_mixture(subset(working_aggregated_mean, species == "FASY"))
print("--------------------- QUSP------------------")
evaluate_mixture(subset(working_aggregated_mean, species == "QUSP"))

## Figure making ####
figure <- ggarrange(plot_sampling_strategy(working_aggregated_mean),
                    plot_management(working_aggregated_mean),
                    plot_mixture(working_aggregated_mean),
                    ...
                    plot_mixture(subset(working_aggregated_mean, species == "ABAL")),
                    ...
                    plot_mixture(subset(working_aggregated_mean, species == "PCAB")),
                    ...
                    plot_mixture(subset(working_aggregated_mean, species == "PISY")),
                    ...
                    plot_mixture(subset(working_aggregated_mean, species == "FASY")),
                    ...
                    plot_mixture(subset(working_aggregated_mean, species == "QUSP")),
                    
                    nrow=6, ncol=3, labels=LETTERS[1:12], 
                    widths = c(0.5,0.25,0.25), align = "hv", common.legend = T, legend = "bottom")