...
# Performs Tukey’s HSD post hoc test to identify which zones differ significantly.
check.differences.crn <- function (sp){
  # sp = "PCAB"
  
  input <- boot.crns(sp)
  
  mod_aov <- aov(mid ~ zone, data = input)
  
  print(TukeyHSD(mod_aov, which = "zone"))
}
...
plot.crns.variability <- function(sp){
  ...
  g <- g + scale_y_continuous("Elevation", limits = c(0.5, 6.5), breaks = elev_y, labels = elev_y)
...
plot.eps <- function(sp){
  ...
  elev_y <- c(1:length(elevation_labels))
  names(elev_y) <- elevation_labels
  input$y <- elev_y[as.character(input$elev_zone)]
  ...
  g<-ggplot(input)
  g <- g + geom_boxplot(aes(x = eps, y = y, fill = elev_zone), alpha = 0.75)
  g <- g + scale_x_continuous("EPS", limits = c(.15, 1.0), breaks=seq(0, 1, 0.1), labels=formatC(seq(0, 1, 0.1),format = "f", digits = 1))
  g <- g + scale_y_continuous("Elevation", limits = c(0.5, 6.5), breaks = elev_y, labels = elev_y)
  g <- g + scale_fill_manual(breaks = names(elev_colors), values = elev_colors)
  g <- my.theme(g)
  g
...
rwi.statistics$elev_zone <- cut(x = rwi.statistics$elevation, 
                                ...
                                labels = elevation_labels)
## Analysis ####
print("---------- between-site variability, differences by elevation zone - ABAL ----------")
check.differences.crn("ABAL")
print("---------- between-site variability, differences by elevation zone - PCAB ----------")
check.differences.crn("PCAB")
print("---------- between-site variability, differences by elevation zone - PISY ----------")
check.differences.crn("PISY")
print("---------- between-site variability, differences by elevation zone - FASY ----------")
check.differences.crn("FASY")
print("---------- between-site variability, differences by elevation zone - QUSP ----------")
check.differences.crn("QUSP")
