...
# Prepare annual mean climate/deposition variables and merge them with
# calendar-year random effects extracted from the mixed-effects model.
...
dataset.cormat <- rbind(prepare.cormat.data(main_mod_dataset, ranef(mod_main)),
...
##------------------------------------------------------------------------------
## RE correlation with environmental variables ####
corr_data <- dataset.cormat %>%
  pivot_longer(
  ...
  dplyr::summarise(
    ...
    p_value = cor.test(
      value[complete.cases(value, RandomEffect)],
      RandomEffect[complete.cases(value, RandomEffect)],
      method = "pearson"
    )$p.value,
    .groups = "drop"
  ) %>%
...
corrmat_graph <- ggplot(corr_data, aes(x = species, y = variable, fill = cor)) +
...
figure<-ggarrange(models, 
                  ggarrange(panel_a, corrmat_graph, nrow=1, ncol=2, labels=LETTERS[6:7], widths=c(0.60,0.40), legend="none"),
